...
To only run tests which have a description that match specific regular
expression run::

    npm test -- -g POST

This will only run tests which have the are in a ``describe('.*POST.*',
function() {`` block (where ``.*POST.*`` is a regular expression. Essentially
the ``-g`` syntax can be treated as a fuzzy-search where only tests wrapped in
a block with the substring you enter are run).
